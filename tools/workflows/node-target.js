...
// Model for every agent in the run; pass args.model to override (an alias like 'sonnet' or a full model id).
...
async function corpusCheck(tag, ph) {
  ...
  const START_CMD = `nohup sh -c '${cmd.replace(/`/g, '')} > ${LOG} 2>&1; echo exit=$? > ${LOG}.done' > /dev/null 2>&1 &`
  for (let i = 1; i <= 24; i++) {
    const r = await agent(`${ENV}\n\nEnsure the conformance sweep for tag ${tag} is running, then wait on it. This is self-healing: run it exactly this way every time, do not skip the check.\n\n1. If ${LOG}.done already exists, skip to step 4.\n2. Otherwise check \`pgrep -af "zig build conformance"\`. If its output is EMPTY (no matching process; do not assume a PID is valid without seeing it in this output), the sweep is not running — start it now with exactly: \`cd ${LUMEN} && ${START_CMD}\`, then immediately \`sleep 3 && pgrep -af "zig build conformance"\` and paste that output in your notes as proof it started. If \`pgrep -af\` already shows a matching process, do nothing.\n3. Wait with a foreground command, at most ten minutes: \`timeout 600 sh -c 'while [ ! -e ${LOG}.done ]; do sleep 20; done'\`.\n4. If ${LOG}.done exists now: read ${LOG}, list every line starting with FAIL, compare with ${LUMEN}/specs/501-node-runtime/corpus_baseline.txt, and report state=done with pass=true iff no FAIL line is absent from the baseline, failures = the new FAIL lines verbatim. If it still does not exist after your step-3 wait, report state=running with pass=false and failures=[] plus the last 'conformance:' line of ${LOG} if the file exists yet, or \"log not created yet\" if it does not — and say in your notes whether step 2 found or started a process, so a stuck sweep is visible. Do not try tools/emit_snapshot.sh (needs two directories to diff; a snapshot regression is already covered by the correctness review's own check). Do not change tracked files.`, { model: MODEL, label: `corpus-wait:${tag}#${i}`, phase: ph, effort: 'low', schema: CORPUS_SCHEMA })
    if (!r) continue
...
for (const spec of SPECS) {
  ...
  for (let round = 1; round <= MAX_ROUNDS; round++) {
    ...
    const [gate, correctness, causes] = await parallel([
      () => agent(`${ENV}\n\n${GATE}\n\nRun the gate for spec ${spec} exactly as listed and report the truth. Also run \`node --test packages/node-runtime/tests/\` if that directory exists. Do not fix anything, do not change files.`, { model: MODEL, label: `gate:${spec.slice(0, 3)}#${round}`, phase: 'Implement', effort: 'low', schema: GATE_SCHEMA }),
      () => agent(`${RULES}\n\nReview the work committed on ${BRANCH} for spec ${spec} (\`git log --oneline origin/main..HEAD -- . \` and \`git diff origin/main...HEAD\` in ${LUMEN}, restricted to what this spec touches) against ${dir}/spec.md through the correctness lens: does each FR hold, does each conformance case pin what the spec says, are the Zig/wasm targets untouched (compare generated Zig for two corpus programs before and after if the emitter was touched)? Try to refute the claim that the spec is satisfied. Report only what you verified by reading or running; mark blocking only for a spec violation. Do not change files.`, { model: MODEL, label: `review-correctness:${spec.slice(0, 3)}#${round}`, phase: 'Implement', effort: 'high', schema: REVIEW_SCHEMA }),
      () => agent(`${RULES}\n\nReview the same diff (\`git diff origin/main...HEAD\` in ${LUMEN}, what spec ${spec} touches) through the CLAUDE.md lens: any workaround in an example/package instead of a fix in the compiler, any fixture migrated to make a suite pass, any weakened or skipped test, any symptom-level fix (a better message for a wrong rule), any missing tasks.md tick or codemap regeneration (\`sh tools/codemap.sh\` must leave docs/CODEMAP.md unchanged). Mark blocking only for those categories. Do not change files.`, { model: MODEL, label: `review-causes:${spec.slice(0, 3)}#${round}`, phase: 'Implement', effort: 'medium', schema: REVIEW_SCHEMA }),
    ])
