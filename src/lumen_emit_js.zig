...
pub const Emitter = struct {
    ...
    /// `Worker.run(fn)` (spec 508 T009). A scalar-capturing arrow has no
    /// module of its own to export from, so its call site synthesizes a
    /// named top-level function and appends its declaration text here;
    /// flushed into the current module's own output once, after its
    /// ordinary statements, since JS `function` declarations hoist within
    /// their scope regardless of where in the file they sit.
    hoisted: std.ArrayListUnmanaged(u8) = .empty,
    worker_synth_seq: usize = 0,
    /// Every name passed by reference to `Worker.run` anywhere in the
    /// program (`refsInExpr`'s `.static_call` case populates this as a side
    /// effect of its ordinary reference walk) — the plain-top-level-function
    /// form. Its own module must export it even when nothing else in the
    /// program imports it, since the spawned worker reaches it only via a
    /// dynamic `import()`, never a normal call.
    worker_run_targets: std.StringHashMapUnmanaged(void) = .empty,
    /// Filled in once `worker_run_targets` is known to be complete (after
    /// every module's references are walked, in `emitProgram`): the output
    /// path of the module that declares each such name.
    worker_target_modules: std.StringHashMapUnmanaged([]const u8) = .empty,
    /// The output path of the module currently being emitted, so a nested
...
fn refsInExpr(e: *Emitter, set: *std.StringHashMapUnmanaged(void), x: *const ast.Expr) CompileError!void {
    switch (x.*) {
        ...
        .static_call => |sc| {
            // `Worker.run(fn)` where `fn` is a plain top-level function
            // (spec 508 T009): recorded here, as a side effect of the
            // ordinary reference walk every call's args already get, so
            // `emitProgram` can force that function's own module to export
            // it even when this is the only reference to it in the program.
            if (std.mem.eql(u8, sc.namespace, "Worker") and std.mem.eql(u8, sc.name, "run") and sc.args.len == 1 and sc.args[0].* == .var_ref) {
                e.worker_run_targets.put(e.arena, sc.args[0].var_ref.name, {}) catch return error.OutOfMemory;
            }
            try refsInExprs(e, set, sc.args);
        },
...
pub fn relativeSpecifier(arena: std.mem.Allocator, from: []const u8, to: []const u8) CompileError![]const u8 {
...
pub fn emitProgram(program: *const ast.Program, arena: std.mem.Allocator, diag: *diag_mod.Diag, warnings: ?*std.ArrayListUnmanaged(diag_mod.Diag), entry: []const u8, line_map: []const diag_mod.LineOrigin, module_paths: []const diag_mod.ModulePath, module_edges: []const diag_mod.ModuleEdge, link_node: []const diag_mod.LinkNodeModule) CompileError!Output {
    ...
    // `Worker.run(fn)` where `fn` names an existing top-level function
    // (spec 508 T009): its own module must export it even in the common
    // case where the call site sits in that same module and nothing else
    // in the program ever imports it — the spawned worker reaches it only
    // through a dynamic `import()`, which sees exported bindings only.
    {
        var wit = e.worker_run_targets.keyIterator();
        while (wit.next()) |name| {
            for (modules.items) |m| {
                if (m.decls.get(name.*) == null) continue;
                m.exports.put(arena, name.*, {}) catch return error.OutOfMemory;
                e.worker_target_modules.put(arena, name.*, m.out) catch return error.OutOfMemory;
            }
        }
    }
    ...
    for (modules.items) |m| {
        ...
        if (e.hoisted.items.len > 0) try e.out.appendSlice(arena, e.hoisted.items);
