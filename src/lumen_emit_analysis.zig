...
pub fn exprCanThrow(e: *const Expr) bool {
    ...
    return switch (e.*) {
        ...
        .non_null => |nn| exprCanThrow(nn.inner),
        ...
        .static_call => |sc| blk: {
            ...
            if (std.mem.eql(u8, sc.namespace, "JSON") and (std.mem.eql(u8, sc.name, "parse") or std.mem.eql(u8, sc.name, "parseOpen"))) break :blk true;
...
pub fn exprUsesThis(e: *const Expr) bool {
    return switch (e.*) {
        ...
        .non_null => |nn| exprUsesThis(nn.inner),
