...
pub const TypeDecl = struct {
    ...
    // Declared with the `interface` keyword. An interface with method members is
    // used polymorphically (a fat pointer + vtable) when classes implement it
    // (spec 428); a data-only interface stays a structural record.
