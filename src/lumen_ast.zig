...
pub const Expr = union(enum) {
    ...
    method_call: struct { obj: *Expr, name: []const u8, args: []*Expr, class_name: ?[]const u8 = null, is_static: bool = false, array_elem_type: ?types.Type = null, array_acc_type: ?types.Type = null, array_result_type: ?types.Type = null, string_method: bool = false, container_type: ?types.Type = null, optional_chain: bool = false, chain_result_type: ?types.Type = null, cb_wants_index: bool = false, number_method: bool = false, is_console: bool = false, regex_arg: bool = false, sized_fill: bool = false, bool_method: bool = false }, // obj.m(args) / Class.m(args) / Map|Set method; is_console: console.log/error/... used as a void expression; regex_arg: a string method (replace) whose first arg is a regex; sized_fill: `new Array(n).fill(v)` fused initializer
    ...
    cast: struct { inner: *Expr, annotation: []const u8, checked_type: ?types.Type = null, is_satisfies: bool = false, float_to_int: bool = false, int_array_to_float: bool = false }, // `expr as T` (safe-subset assertion; erased at emit). is_satisfies: `expr satisfies T` (spec 052) -- checks assignability to T but keeps expr's own narrower type. float_to_int: `<f64> as i32/i64` truncates at emit. int_array_to_float: `i32[]` value flowing into a `number[]` slot -> elementwise @floatFromInt copy (spec 415)
};
