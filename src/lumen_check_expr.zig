...
pub fn exprType(self: *Checker, program: *ast.Program, e: *ast.Expr, line: u32, col: u32) ?types.Type {
    return switch (e.*) {
        ...
        .method_call => |*mc| {
            ...
            if (types.isAsyncSocket(obj_type)) {
                return self.asyncSocketMethod(program, mc, obj_type, line, col);
            }
            ...
            if (types.isAsyncResponseWriter(obj_type)) {
                return self.asyncResponseWriterMethod(program, mc, obj_type, line, col);
            }
